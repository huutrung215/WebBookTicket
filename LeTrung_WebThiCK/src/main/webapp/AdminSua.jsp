...
<section class="container">
	<form method="post" action="AdminSuaPhimController" enctype= "multipart/form-data">
	    <c:forEach items="${liP}" var="o">
		<div class="mb-3 mt-3">
		    <label for="id" class="form-label">ID phim:</label>
		    <input type="text" id="id" value="${o.id}"  class="form-control" name="idphim" readonly/>
		 </div>
	  <div class="mb-3 mt-3">
	    <label for="tenphim" class="form-label">Tên phim:</label>
	    <input type="text" id="tenphim" value="${o.tenphim}" class="form-control" name="tenphim"/>
	  </div>
	  <div class="mb-3">
	    <label for="chonanh" class="form-label">Chọn ảnh:</label>
	     <input id="chonanh" type="file" class="form-control" value="${o.getAnh()}"  name="anh"/>
	  </div>
	  <div class="mb-3 mt-3">
	    <label for="loai" class="form-label">Thể loại:</label>
	    <input type="text" id="loai" value="${o.theloai}" class="form-control" name="theloai"/>
	  </div>
	  <button type="submit" name="butsua" class="btn btn-primary">Submit</button>
	  </c:forEach>
  </form> 
</section>
</body>
